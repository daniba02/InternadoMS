package Presentacion.Vista.Academia.Asignatura.GUIAltaAsignatura;
...
public class GUIAltaAsignatura extends JFrame implements IGUI {
	...
	public enum Tipo {
		NORM, OBLIGATORIA, OPTATIVA
	};

	Tipo tipo = Tipo.NORM;

	public GUIAltaAsignatura() {
		super("ALTA ASIGNATURAS");
		initGUI();
	...
	public void initGUI() {
		...
		combo.setModel(new DefaultComboBoxModel<String>(new String[] { " Elija un tipo: ", "Obligatoria", "Optativa" }));
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (tipo == Tipo.OBLIGATORIA) {
						...
						ApplicationController.getInstancia().handleRequest(new Context(Evento.AltaAsignaturas,
								new TObligatoria(-1, true, tNombre.getText(), tOblig.getText())));
						...
						ApplicationController.getInstancia().handleRequest(
								new Context(Evento.AltaAsignaturas, new TOptativa(-1, true, tNombre.getText(), nivel)));
					} else {
	...
	public void actualizar(Context context) {
		// begin-user-code
		if (context.getEvento() == Evento.AltaAsignaturas_OK) {
