package Presentacion.Vista.Academia.Asignatura.GUIListarAsignaturasObligatorias;
...
public class GUIListarAsignaturasObligatorias extends JFrame implements IGUI {

	private static final long serialVersionUID = 1L;
	private DefaultTableModel _dataTableModel;
	private String[] headers = { "ID Asignatura", "Nombre", "Itinerario", "Activo" };
	private Collection<TObligatoria> listaAsig;
	JPanel panelPrincipal;

	public GUIListarAsignaturasObligatorias() {
		super("LISTAR ASIGNATURAS OBLIGATORIAS");
		initGUI();
	...
	public void actualizar(Context context) {
		if (context.getEvento() == Evento.ListarAsignaturasObligatorias_OK) {
			int n = 0;
			listaAsig = (Collection<TObligatoria>) context.getDato();
			if (listaAsig != null) {
				_dataTableModel.setNumRows(listaAsig.size());
				for (TObligatoria p : listaAsig) {

					_dataTableModel.setValueAt(p.getID(), n, 0);
					_dataTableModel.setValueAt(p.getNombre(), n, 1);
					_dataTableModel.setValueAt(p.getItinerario(), n, 2);
