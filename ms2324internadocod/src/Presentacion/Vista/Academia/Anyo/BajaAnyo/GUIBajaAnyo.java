 ...
 * 
 */
package Presentacion.Vista.Academia.Anyo.BajaAnyo;
...
public class GUIBajaAnyo extends JFrame implements IGUI {
	...
	public void initGUI() {
		...
		JLabel ID = new JLabel(" ID del anio: ");
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (tID.getText().equals("")) {
						throw new Exception("No puedes dar de baja un anio si no proporcionas un id.");
					}
					if (!aux.matches("^[0-9]*$"))
						throw new Exception("El id debe ser un numero entero positivo");
					id = Integer.parseInt(aux);

					ApplicationController.getInstancia().handleRequest(new Context(Evento.BajaAnio, id));
				} catch (Exception ex1) {
	...
	public void actualizar(Context context) {
		if (context.getEvento() == Evento.BajaAnio_OK) {
