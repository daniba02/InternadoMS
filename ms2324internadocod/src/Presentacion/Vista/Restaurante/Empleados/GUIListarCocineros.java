...
public class GUIListarCocineros extends JFrame implements IGUI{

	private static final long serialVersionUID = 1L;
	
	private DefaultTableModel _dataTableModel;
	private String[] headers = {"Id Empleado", "Nombre", "Nomina", "Horas", "ID Turno", "Especialidad", "Factor", "Activo"};
	private Collection<TCocinero> lista;
	JPanel panelPrincipal;
	
	public GUIListarCocineros() {
		super("LISTAR COCINEROS");
		initGUI();
	...
	public void actualizar(Context context) {
		...
		if (context.getEvento() == Evento.ListarCocineros_OK) {
			int n = 0;
			lista = (Collection<TCocinero>) context.getDato();
			if (lista != null) {
				_dataTableModel.setNumRows(lista.size());
				for (TCocinero Co : lista) {

					_dataTableModel.setValueAt(Co.getId(), n, 0);
					_dataTableModel.setValueAt(Co.getNombre(), n, 1);
					_dataTableModel.setValueAt(Co.getSueldoPorHora(), n, 2);
					_dataTableModel.setValueAt(Co.getHoras(), n, 3);
					_dataTableModel.setValueAt(Co.getTurno(), n, 4);
					_dataTableModel.setValueAt(Co.getEspecialidad(), n, 5);
					_dataTableModel.setValueAt(Co.getFactor(), n, 6);
					_dataTableModel.setValueAt(Co.getActivo(), n, 7);
