...
public class GUIMostrarVentasPorCliente extends JFrame implements IGUI {
	...
	public GUIMostrarVentasPorCliente() {
		super("LISTAR VENTAS POR CLIENTE");
		initGUI();
	...
	public void initGUI() {
		...
		JLabel Cliente = new JLabel(" Nombre Cliente: ");
		...
		panel3_2.add(Cliente,BorderLayout.PAGE_START);
		panel3_1.add(tCliente,BorderLayout.PAGE_START);
		...
		aceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					ApplicationController.getInstancia().handleRequest(new Context(Evento.ListarVentasPorCliente,tCliente.getText()));					
				} catch (Exception e1) {
					JOptionPane.showMessageDialog(null, "Hay que especificar cliente correcto (String)", "", JOptionPane.ERROR_MESSAGE);
				}
				finally
				{
					setVisible(false);
	...
	public void actualizar(Context context) {
		if(context.getEvento()==Evento.ListarVentasPorCliente_OK) {
			int it = 0;
