...
public class ListarBebidas extends JFrame implements IGUI {

	private static final long serialVersionUID = 1L;
	private DefaultTableModel _dataTableModel;
	private String[] headers = {"Id Plato", "Nombre", "Stock","Precio", "Activo", "Temperatura"};
	private Collection<TBebida> listabebida;
	JPanel panelPrincipal;

	public ListarBebidas() {
		super("LISTAR BEBIDAS");
		initGUI();
	...
	public void actualizar(Context context) {
		...
		if (context.getEvento() == Evento.ListarBebidas_OK) {
			int n = 0;
			listabebida = (Collection<TBebida>) context.getDato();
			if (listabebida != null) {
				_dataTableModel.setNumRows(listabebida.size());
				for (TBebida b : listabebida) {

					_dataTableModel.setValueAt(b.getPlato().getId(), n, 0);
					_dataTableModel.setValueAt(b.getPlato().getNombre(), n, 1);
					_dataTableModel.setValueAt(b.getPlato().getStock(), n, 2);
					_dataTableModel.setValueAt(b.getPlato().getPrecio(), n, 3);
					_dataTableModel.setValueAt(b.getPlato().getActivo(), n, 4);
					_dataTableModel.setValueAt(b.getTemperatura(), n, 5);
					n++;
				}
			}
